...
function nextCalendar() {
	...
	today = new Date(nextMonth.getFullYear(), nextMonth.getMonth(), Math.min(today.getDate(), getLastDay(nextMonth))); 
    buildCalendar();
}

function getLastDay(date) {
	return new Date(date.getFullYear(), date.getMonth() + 1, 0).getDate(); // 해당 달의 마지막 일을 반환
}
...
// 페이지가 로드되면 달력 테이블 구현, 시간 테이블 구현 함수 호출
...
// 초기화 버튼을 누르면 모든 input태그 초기화 시켜주는 함수
...
<body>
	...
	<!-- post 방식으로 servlet 맵핑 submit 버튼 눌렀을 때 makeReservation()함수 호출 -->
			...
			<div class="notice-wrapper">
				<div class="notice-box">
					...
					<ul>
						...
						<li>※당일 예약취소는 불가능하니 주의 부탁드립니다.※</li>
					</ul>
				</div>
			</div>
		</section>
