							...
							%> <c:choose>
								<c:when test="${ dto.icomment == null }">
									<button type="button" class="btn btn-secondary btn-lg"
										onclick="location.href='../mvc2/edit.do?num=${ param.num }&virtualnum=${ param.virtualnum }';">
										수정하기</button>
									<button type="button" class="btn btn-secondary btn-lg"
										onclick="location.href='../mvc2/delete.do?num=${ param.num }';">
										삭제하기</button>
								</c:when>
							</c:choose> <%
 	}
 %> 
 <!--  어드민일 경우 답변, 삭제 여기서는 어드민이 맞는지 아닌지만 판별 -->
 <%
 	if (session.getAttribute("UserNik").equals("admin")) {
 %>
							<button type="button" class="btn btn-secondary btn-lg"
